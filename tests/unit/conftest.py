    ...
    async def _add_users() -> None:
        async with async_session_maker() as session:
            for user_schema in users:
                await session.execute(insert(UserModel).values(**user_schema.model_dump()))
            await session.commit()
